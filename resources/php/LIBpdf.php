 ...
 * GPL Ghostscript 9.19: **** Could not open temporary file
    /var/folders/r0/6br2l3h52nzgjtw30g7sdcfw0000gn/T/gs_C8J9yA
 */
...
class PDFLib
{
    ...
    public function __construct()
    {
        ...
        if ($gs_version == -1) {
            ...
            throw new \Exception("Your version of GhostScript $gs_version is not compatible
         with the library", 403);
        }
    }
    /** * Set the path to the PDF file to process * @param string $pdf_path * @return
         self */ public function setPdfPath($pdf_path)
    {
    ...
    public function getNumberOfPages()
    {
        if ($this->number_of_pages == -1) {
            ...
            $pages = $this->executeGS('-q -dNODISPLAY -c "(' . $this->pdf_path . ') (r) file runpdfbegin pdfpagecount =
         quit"', true);
    ...
    public function convert()
    {
        ...
        $output = $this->executeGS("-dSAFER -dBATCH -dNOPAUSE -sDEVICE=" . $this->imageDeviceCommand . "
                         " . $this->pngDownScaleFactor . " -r" . $this->resolution . " -dNumRenderingThreads=4
                         -dFirstPage=" . $this->page_start . " -dLastPage=" . $this->page_end . " -o\"" . $image_path .
            "\" -dJPEGQ=" . $this->jpeg_quality . " -q \"" . ($this->pdf_path) . "\" -c quit");
    ...
    public function makePDF($ouput_path_pdf_name, $imagePathArray)
    {
        ...
        $command = '-dBATCH -dNOPAUSE -sDEVICE=pdfwrite -o"' . $ouput_path_pdf_name . '" "' .
            $psfile . '" -c "' . $imagesources . '"';
    ...
    private function initSystem()
    {
        ...
        if ($this->gs_path == null || $this->gs_version == null || $this->gs_is_64 == null) {
            if ($this->is_os_win) {
                error_log("Windows OS detected" . ($this->execute("where gswin64c.exe", true) ? "64 bit" : "32 bit"));
                if (!empty(trim($gs_bin_path = $this->execute("where gswin64c.exe", true)))) {
                    $this->gs_is_64 = true;
                    $this->gs_command = "gswin64c.exe";
                    $this->gs_path = trim(str_replace("bin\\" . $this->gs_command, "", $gs_bin_path));
                } else if (!empty(trim($gs_bin_path = $this->execute("where gswin32c.exe", true)))) {
                    $this->gs_is_64 = false;
                ...
                if (!((is_array($output) && (strpos($output[0], 'is not recognized as an internal or
                             external command') !== false)) || !is_array($output) && trim($output) == "")) {
                    $this->gs_command = "gs";
                    $this->gs_version = doubleval($output[0]);
